...
def processar_e_unificar_dados(df_rm_bruto, df_pc_bruto, df_vinculo, buscar_rm, buscar_pc, filtro_req, filtro_comp, filtro_status_pc, filtro_periodo):
    ...
    # 1. Limpeza e estruturação estrita da tabela de RMs
    ...
    # 2. Limpeza e estruturação estrita da tabela de Vínculos intermediários
    ...
    # 3. Limpeza e Sintonização Absoluta da tabela de Pedidos (Alinhada com a sua View SQL)
    df_pc_limpo = pd.DataFrame(index=df_pc_bruto.index)
    if not df_pc_bruto.empty:
        df_pc_bruto_copy = df_pc_bruto.copy()
        
        # 🌟 MAPEAMENTO CIRÚRGICO NATIVO: Transforma os campos reais da sua View nas colunas do layout
    ...
    # 🌟 CRUZAMENTO FLEXÍVEL POR MATERIAL DE HOJE DE MANHÃ: Garante dados na tela!
    df_final = pd.merge(df_rm_consolidada, df_pc_limpo, on=["mat_str"], how="outer")

    # Consolida e recupera o número do pedido de forma limpa pós-merge
    ...
    # Filtros de interface em memória RAM rápidos e responsivos
    if buscar_rm:
