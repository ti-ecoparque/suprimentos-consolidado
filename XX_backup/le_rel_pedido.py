...
def processar_e_salvar_dados():
    ...
    try:
        ...
        for coluna_banco in mapeamento_colunas.values():
            df_filtrado[coluna_banco] = (
            df_filtrado[coluna_banco]
            .fillna("")
            .astype(str)
            .str.strip()
        )
        # Converte Quantidade para numero 
        df_filtrado["quantidade"] = (
            pd.to_numeric(df_filtrado["quantidade"], errors="coerce")
            .fillna(0)
        )    
